...
# create long format
pheno_data_long_above_below <- pivot_longer(pheno_data_final, cols = c("DW_Roots_Total", "DW_Shoot"), names_to = "Part", values_to = "Part_DW")

# set treatment labels
treatment_labels_above_below <- c("Shoot", "Root")

# set plot order
pheno_data_long_above_below$Part <- factor(pheno_data_long_above_below$Part, 
                                           levels = c("DW_Shoot", "DW_Roots_Total"))
treatment_order_above_below <- c("Shoot", "Root")

# create plot
dw_above_below_plot <- ggplot(pheno_data_long_above_below, aes(x = Part, y = Part_DW, fill = Treatment)) +
  geom_boxplot(outlier.shape = 16, outlier.size = 2, alpha = 0.2, lwd = 0.5, aes(color = Treatment)) +
  scale_fill_manual(values = custom_colors, labels = treatment_labels, breaks = treatment_order) +
  scale_color_manual(values = custom_colors, labels = treatment_labels, breaks = treatment_order) +
  scale_x_discrete(labels = treatment_labels_above_below) +
  geom_signif(
    y_position = c(0.98, 1.05, 0.86, 0.93), xmin = c(0.906, 0.906, 1.906, 1.906), xmax = c(1.094, 1.282, 2.0935, 2.282),
    annotation = c("< 0.001", "0.012", "< 0.001", "< 0.001")
  ) +
  labs(title = "C    Shoot vs. Root Biomass Produced",
       subtitle = "        Only significant p-values are shown (Wilcoxon test with Bonferroni correction) ",
       x = "Plant Part",
       y = "Dry Weight (g)"
  ) +
...
# create long format
pheno_data_long_allocation <- pivot_longer(pheno_data_final, cols = c("DW_Shoot", "DW_Roots_Total"), names_to = "Part", values_to = "Part_DW")
...
# calculate GLMs (sum)
model_DW_Shoot <- glm(Part_DW ~ Treatment + DW_Plant, data = subset_DW_Shoot, family = gaussian())
model_DW_Roots <- glm(Part_DW ~ Treatment + DW_Plant, data = subset_DW_Roots, family = gaussian())

# check distributions of residuals
hist(residuals(model_DW_Shoot))
qqnorm(residuals(model_DW_Shoot)); qqline(residuals(model_DW_Shoot))
hist(residuals(model_DW_Roots))
qqnorm(residuals(model_DW_Roots)); qqline(residuals(model_DW_Roots))
...
emm_DW_Shoot <- emmeans(model_DW_Shoot, ~ Treatment | DW_Plant, type = "response",adjust="sidak")
plot(emm_DW_Shoot, comparisons = TRUE)
emm_DW_Shoot_results <- as.data.frame(emm_DW_Shoot)
emm_DW_Shoot_results$Part <- "Shoot"
emm_DW_Roots <- emmeans(model_DW_Roots, ~ Treatment | DW_Plant, type = "response",adjust="sidak")
...
write.csv(emm_results, "results/Figure1/emm_resource_allocation.csv", row.names = FALSE)
...
write.csv(emm_pairwise, "results/Figure1/pairwise_comparison_resource_allocation.csv", row.names = FALSE)
...
# set treatment order
# Or alternatively, create a new mapping
...
# create plot
emm_plot <- ggplot() +
  geom_point(data = emm_results, aes(x = Part, y = emmean, color = Treatment), 
             position = position_dodge(width = 0.75), size = 3) +
  geom_linerange(data = emm_results, aes(x = Part, y = emmean, color = Treatment, 
                                         ymin = lower.CL, ymax = upper.CL), 
                 position = position_dodge(width = 0.75), size = 1.2) +
  geom_beeswarm(data = pheno_data_long_allocation, 
                aes(x = Part, y = Part_DW, color = Treatment), alpha = 0.25, dodge.width = 0.75, shape = 16) +
  scale_color_manual(values = custom_colors, labels = treatment_labels, breaks = treatment_order) +
  scale_x_discrete(labels = c("Shoot", "Root")) +
  labs(title = "D    Treatment Effects on Plant Biomass Allocation",
...
ggsave("results/Figure1/Figure1_R.pdf", plot = combined_plot_pheno, width = 12.5, height = 15)
dev.off()
